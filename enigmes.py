...
def charger():
    global var_sauv, argent, points, e_resol, e_ab, ct, espace, e, nb_e, indice_fournis, limite_indices, ameliorations, niv_, essais_, pt, ar_
    fichier = "fich_sauv"
    if os.path.isfile(fichier):
        ...
        limite_indices = var_sauv[9]
        ameliorations = var_sauv[10]
        niv_ = var_sauv[11]
        essais_ = var_sauv[12]
        pt = var_sauv[13]
        ar_ = var_sauv[14]
...
def tirer_enigme():
    ...
    if len(nb_e) == 0:
        ...
        if e in (2, 4, 6, 9, 11):
            ...
            sauver(argent, points, e_resol, e_ab, ct, espace, e, nb_e, indice_fournis, limite_indices, ameliorations, niv_, essais_, pt, ar_)
...
def repondre(niv, essais, pt, ar):
    ...
    global argent
    rep = input()
    if rep in reponses['rep e%s' % e] or rep == 'triche':
        saut()
        print("Bonne réponse !!!!")
        ct = 0
        points = points + pt
        ...
        if indice_fournis in (0, 1):
            saut()
            print(indices['i %s' % e])
            if indices['i %s' % e] == "Cette énigme est une énigme de niveau %s, par conséquent il n'y a pas d'indice disponible" % niv:
                pass
            else:
                indice_fournis += 1
        elif indice_fournis == limite_indices:
            saut()
        ...
        while rep != reponses['rep e%s' % e]:
            ct = ct + 1
            saut()
            print("Mauvaise réponse !")
            saut()
            rep = input()
            if rep in reponses['rep e%s' % e]:
                saut()
                print("Bonne réponse !!!!")
                if ct > essais:
...
class commande():
    ...
    def info():
        global limite_indices
        global ameliorations
        saut()
        if points >= 10 and points < 20:
            ...
            if ameliorations == 2:
                ...
                print("BRAVO !! Vous avez atteint les 20 points ou plus !\nVous débloquez 2 indice suplémentaire !!")
                limite_indices += 2
        print("Vos statistiques :\n\npoints : %s\nnb d'énigme résolues : %s\nenigmes abandonnées : %s\nvotre argent : %s$\nindices disponibles : %s\nindices utilisés : %s" % (points, e_resol, e_ab, argent, (limite_indices - indice_fournis), indice_fournis))
        saut()
        sauver(argent, points, e_resol, e_ab, ct, espace, e, nb_e, indice_fournis, limite_indices, ameliorations, niv_, essais_, pt, ar_)
        commande.help()
    def leave():
